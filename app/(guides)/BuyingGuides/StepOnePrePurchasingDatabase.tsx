...
const StepOnePrePurchasingDatabase = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubSteps = async () => {
      let allSubSteps: SubStep[] = [];

      for (let i = 0; i < 6; i++) {
        try {
          const response = await axios.get(
            `http://192.168.125.14:3000/api/substeps?step_number=${i}&include_bullets=true`
          );
    ...
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(
          `http://192.168.125.14:3000/api/questions?step_number=1`
        );

        // Directly map the questions without parsing options
...
export default StepOnePrePurchasingDatabase;
